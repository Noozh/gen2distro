...
#define PARTY_SIZE 6
#define TRAINER_NAME_COUNT 6
#define NICKNAME_COUNT 6
#define MOVE_COUNT 4

#define PLAYER_LENGTH_GEN_II 444

#define RANDOM_SEED_SIZE                10
#define PARTY_DATA_SIZE                 418
#define PATCH_SIZE                      202
// Before this position all is filled with 0x00.
#define PATCH_DATA_START_POS            7
          
#define PARTY_DATA_SIZE_GEN2 444
#define MAIL_DATA_SIZE_GEN2 381
#define PATCH_SIZE_GEN2 202
// Protocol opcodes.
...
#define PKMN_TIME_CAPSULE_SELECT        ITEM_1_SELECTED
...
// Struct for gen 2 Pokemon based on https://bulbapedia.bulbagarden.net/wiki Pokémon_data_structure_(Generation_II)
// Total bytes : 48 bytes
typedef struct {
    uint8_t species; // 1 byte
    uint8_t heldItem; // 1 byte
    uint8_t moves[MOVE_COUNT]; // 4 bytes
    uint16_t otID; // 2 bytes
    uint8_t exp_1; // 1 byte
    uint8_t exp_2; // 1 byte
    uint8_t exp_3; // 1 byte
    uint16_t evHP; // 2 bytes
    uint16_t evAttack; // 2 bytes
    uint16_t evDefense; // 2 bytes
    uint16_t evSpeed; // 2 bytes
    uint16_t evSpecial; // 2 bytes
    uint16_t ivData; // 2 bytes
    uint8_t movePP[MOVE_COUNT]; // 4 bytes 
    uint8_t friendship; // 1 byte
    uint8_t pokerus; // 1 byte
    uint8_t captureData; // 2 bytes
    uint8_t level; // 1 byte
    uint8_t status; // 1 byte 
    uint8_t unused_byte; // 1 byte
    uint16_t currentHP; // 2 bytes
    uint16_t maxHP; // 2 bytes
    uint16_t attack; //2 bytes
    uint16_t defense; // 2 bytes
    uint16_t speed; //2 bytes
    uint16_t specialAttack; // 2 bytes
    uint16_t specialDefense; // 2 bytes
} PokemonGen2;


// Struct for gen 2 trainer
// Total bytes: 444
typedef struct {
    char name[NAME_LENGTH]; // Trainer Name 									11 bytes
    uint8_t partyCount; // Pokemon in party 									1 byte
    uint8_t partyIDs[PARTY_SIZE]; // Pokemon party ID 								6 bytes
    
    uint8_t stopByte; // Stop byte 										1 byte
    
    uint16_t trainerID; // Trained ID										2 bytes
    PokemonGen2 party[PARTY_SIZE]; // Party member def								48 * 6 bytes = 288 bytes
    char trainerNames[TRAINER_NAME_COUNT][NAME_LENGTH];	// Pokemon party trainer name				11 * 6 bytes = 66 bytes
    char nicknames[NICKNAME_COUNT][NAME_LENGTH]; // Pokemon party nickname					11 * 6 bytes = 66 bytes
    uint8_t unusedBytes[3]; // Unused bytes									3 bytes
} PlayerData;
